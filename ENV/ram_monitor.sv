...
class ram_monitor;
  ...
  //connect
  function void connect(mailbox#(ram_trans) mon2rm_mbx, mailbox#(ram_trans) mon2scb_mbx,
                        virtual ram_if.MON_MP vif);
  ...
  task dut2monitor();
    ...
    if (item_collected.we && !item_collected.re) item_collected.kind_e = WRITE;

    else if (!item_collected.we && item_collected.re) item_collected.kind_e = READ;

    else if (item_collected.we && item_collected.re) item_collected.kind_e = SIM_RW;

    else item_collected.kind_e = IDLE;
